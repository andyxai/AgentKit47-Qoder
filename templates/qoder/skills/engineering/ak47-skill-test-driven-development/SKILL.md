...
**来源**: 融合社区 TDD 实践与 mattpocock-skills/tdd
...
- **单元测试**: 按项目框架约定放置（如 `tests/`、`__tests__/`、或与源码同目录的 `*.test.*`）
- **集成/E2E 测试**: 放入独立子目录（如 `tests/e2e/`、`tests/integration/`）
- **请勿混放**: 不同类型测试不加区分地放在同一目录

### 产物规范

测试脚本的输出格式必须遵循：

- **标准输出**: 仅输出一行摘要（通过/失败，数量，关键指标）
- **详细数据**: 保存为结构化数据文件（JSON/YAML），与测试脚本同名
- **❌ 禁止**: 创建 Markdown 测试报告文档或格式化表格输出
- **测试结论**: 在对话中给出，不持久化为文档
...
- [ ] 测试文件放哪个目录？需要哪些类型的测试（单元/集成/E2E）？
- [ ] 输出格式是否需要额外的数据处理文件？
- [ ] E2E 测试的外部数据是否需要通过搜索引擎验证获取？（禁止硬编码推测 URL）
...
[ ] 没有添加投机功能
[ ] 测试文件放在正确的目录下
[ ] 没有创建 Markdown 测试报告文档
[ ] 输出格式符合规范（一行摘要 + JSON 数据文件）
```
...
**版本**: 1.0.0  
**来源**: 社区 TDD 实践 + mattpocock-skills/tdd  
