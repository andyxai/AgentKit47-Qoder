...
## 🚫 Pre-check：禁止 AI 自我审查

> **新增于 2026-05-14**，解决 AI 自行"扫一眼 + 顺手修 bug"替代正式审查流程的问题。
> **配套**: `.qoder/rules/workflow-state.md`「增量修改审查门禁」+ `.ak47/workflow-rules.yaml` `artifact-modification-review-gate`

**触发条件**: AI 对已审查通过的 artifacts（proposal/design/tasks/specs/*/spec.md）做了任何语义修改。

### 强制行为

1. 必须调用 `ak47-skill-critical-review` 或委托 `ak47-agent-reviewer` 执行增量审查
2. ❌ 禁止 AI 自行判断"改动小不需要审查"
3. ❌ 禁止 AI 自行扫描修复后直接放行（即使发现并修复了问题也不替代审查）
4. ❌ 禁止 AI 替代审查者角色（审核员不是修改者）
5. 审查结论必须呈现给用户，等待显式批准

### 正确做法

```
> "检测到 tasks.md 已被修改（S5/S9 拆分）。
>  按照增量修改门禁，需要重新执行 critical-review。正在调用审查流程…"
```

### 错误做法（红线）

```
> "我扫了一眼，没发现问题，已顺手修了一个不一致，通过。"
```
↑ 这是在绕过审查——用"肉眼扫一遍 + 顺手修 bug"替代了正式审查流程。

### 与正常 critical-review 的区别

| 维度 | 正常审查（首次） | 增量审查（修改后） |
|------|---------------|------------------|
| 触发时机 | artifacts 首次全部完成 | 修改已审查通过的 artifact |
| 审查范围 | 全量（所有 artifacts） | 增量（本次修改 + 影响的下游 artifacts） |
| 执行要求 | 完整审查流程 | 完整审查流程（不可简化！） |
| 禁止行为 | 跳过审查 | 跳过审查 + AI 自审替代 |

**关键**: 增量审查 ≠ 简化审查。流程强度与首次审查完全相同。
...
- ❌ 审核报告未记录剩余风险
- ❌ 用户未明确确认"可以通过"
- ❌ AI 自行"扫一眼"替代正式审查（新增 2026-05-14）
- ❌ AI 修改 artifact 后自行修复不一致并声明通过（新增 2026-05-14）
- ❌ 增量修改后以"改动小"为借口跳过审查（新增 2026-05-14）
